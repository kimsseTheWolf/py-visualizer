...
<template>
    <div class="main-block" :id="props.id">
        ...
        <template v-for="i in contents">
            ...
            <NestedCommandPanel v-if="i.type === 'param' && (i.acceptTypes[0] === 'nested' && i.acceptTypes.length === 1)" :blocks="getNestedSlotValue(i.index)" :index="i.index" @on-content-change="handleOnValueChange" :scope="props.scope+1"></NestedCommandPanel>
        </template>
